...
def filter_formatted_outputs(input_path="outputs/Qwen2.5-7B-Instruct/gsm8k/7b/Original/samples/predictions_correct.jsonl",
    ...
    formatted_data = []
    for i in range(len(data)):
        if data[i]['cot_length'] > 1500: # TODO: check whether this is actually needed (probably not).
            continue
        ...
        if model_type == "llama3":
            ...
            if len(spans) == 2:
                ...
                """
            if "\\boxed{" in data[i]["model_output"]:
                data[i]["cot"] = data[i]["model_output"].split('\\boxed{')[0].strip()
            else:
                data[i]["cot"] = data[i]["model_output"].strip()
...
def compress_cot_outputs(input_path="outputs/Qwen2.5-7B-Instruct/gsm8k/7b/Original/samples/predictions_formatted.jsonl",
                        output_dir="outputs/LLaMA-3.1-8B-Instruct/piqa/8b/Compression", model_type="qwen",
...
# if __name__ == '__main__':
  #  data_processing_gsm8k(
  #      input_dir="outputs/LLaMA-3.1-8B-Instruct/math/8b/",
  #      model_type="qwen",
...
if __name__ == '__main__':
    ...
    model_sizes = [3, 7, 14]
    datasets = ["boolq", "gsm8k", "math", "piqa"]
    
    llmlingua_model_path = "microsoft/llmlingua-2-xlm-roberta-large-meetingbank"

    for x in model_sizes:
        for YYYY in datasets:
            current_input_dir = f"outputs/Qwen2.5-{x}b-Instruct/{YYYY}/{x}b/"
            
            # Paths to check
            predictions_file = os.path.join(current_input_dir, "Original/train/samples/predictions.jsonl")
            compressed_dir = os.path.join(current_input_dir, "Compression")
            
            # 1. Check whether it was already processed COMPLETELY (exactly 9 files for the 9 ratios)
            if os.path.exists(compressed_dir) and len(os.listdir(compressed_dir)) == 9:
                print(f"\n[ALREADY FULLY PROCESSED - SKIPPED] Model {x}B | Dataset: {YYYY}")
                print(f"All 9 compressed files already exist in: {compressed_dir}")
                continue
            elif os.path.exists(compressed_dir) and len(os.listdir(compressed_dir)) > 0:
                files_found = len(os.listdir(compressed_dir))
                print(f"\n[WARNING - PARTIAL PROCESSING] Model {x}B | Dataset: {YYYY}")
                print(f"Found {files_found} of 9 files. Processing will restart and overwrite the files.")
            
            if os.path.exists(predictions_file):
                print(f"\n========================================================")
                print(f"Starting processing for: Model {x}B | Dataset: {YYYY}")
                print(f"Directory: {current_input_dir}")
                print(f"========================================================\n")
                
                data_processing_gsm8k(
                    input_dir=current_input_dir,
                    model_type="qwen",
                    llmlingua_path=llmlingua_model_path
                )
            else:
                print(f"\n[MISSING DATA - SKIPPED] File not found for Model {x}B | Dataset: {YYYY}")
                print(f"Missing path: {predictions_file}")