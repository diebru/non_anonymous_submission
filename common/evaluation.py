...
def infer(args, test_data, answer_extraction_fn):
            ...
            if mess['role'] == 'user':
                ...
                if args.model_type == 'llama3':
                    if args.compression_ratio < 1.0:
                        prompt += "<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\nPlease reason step by step, and put your final answer within \\boxed{}.\n" + f"{mess['content']}<|eot_id|>{args.compression_ratio}<|eot_id|><|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
                    else:
                        prompt += "<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\nPlease reason step by step, and put your final answer within \\boxed{}.\n" + f"{mess['content']}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
    ...
    for item, output in tqdm(zip(test_data, model_outputs), desc="extract answer", total=len(model_outputs)):
        ...
        pred = eval(answer_extraction_fn)(item['messages'][-2]['content'], output, task='cot')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--benchmark", type=str, choices=['gsm8k', 'math', 'piqa', 'boolq'], default="gsm8k")
    ...
    # Scale the tokens by the ratio to avoid wasting memory
    if args.compression_ratio < 1.0:
    ...
    if args.compression_ratio < 1.0:
        print(f"TokenSkip mode enabled | Compression ratio: {args.compression_ratio}", flush=True)
        args.output_dir = os.path.join(args.output_dir, f"{args.model_size}/", f"TokenSkip/", f"{args.compression_ratio}/")
    else:
        print(f"Baseline mode enabled", flush=True)
    ...
    for src, info in test_conf.items():
        ...
        for item in results:
            if len(item['prediction']) == 0:
                ...
                # PIQA and BoolQ need only an exact string match (e.g. "1" == "1")
                if args.benchmark in ['piqa', 'boolq']:
                    res = (str(item['prediction']).strip() == str(item['answer']).strip())
                else:
                    res = eval_math(item)
            labels.append(res)
        ...
        overall_accuracy = acc / len(results)
        print("output acc = {:.5f}".format(overall_accuracy * 100), flush=True)
        ...
        with open(os.path.join(output_dir, metric_fname), "w") as fout:
            json.dump(metrics_data, fout, indent=4)