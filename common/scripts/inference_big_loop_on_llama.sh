...
# Lists to iterate over (set to 8B for Llama 3.1)
MODELS=("8B") 
BENCHMARKS=("boolq" "gsm8k" "math" "piqa") 

for SIZE in "${MODELS[@]}"; do
    
    MODEL_SIZE="${SIZE,,}" # Convert to lower case (e.g. "8b")
    
    BASE_MODEL_PATH="meta-llama/Llama-3.1-${SIZE}-Instruct"
    
    for BENCHMARK in "${BENCHMARKS[@]}"; do

        echo "===================================================================="
        echo " MODEL: Llama-3.1-${SIZE}-Instruct | BENCHMARK: ${BENCHMARK^^} "
        echo "===================================================================="

        MERGED_MODEL_PATH="${MERGED_MODELS_DIR}/Merged-Llama-3.1-${SIZE}-${BENCHMARK}"
        ...
        for COMPRESSION_RATIO in 1.0 0.9 0.8 0.7 0.6 0.5 0.4 0.3 0.2 0.1; do
            ...
            OUTPUT_DIR="outputs/inference/Llama3.1_${MODEL_SIZE}/${BENCHMARK}/ratio_${COMPRESSION_RATIO}/"
            mkdir -p "${OUTPUT_DIR}"
            ...
            # --- MODEL AND TOKENIZER HANDLING (METHOD 1) ---
            ...
            if [ "$COMPRESSION_RATIO" != "1.0" ]; then
                ...
                if [ ! -d "$CURRENT_MODEL_PATH" ]; then
                    echo "    [ERROR] The merged-model folder does NOT EXIST: $CURRENT_MODEL_PATH"
                    echo "    Skipping this test. Check that the merge completed successfully."
        ...
        echo " All ratio completed for ${BENCHMARK} on Llama 3.1 ${SIZE}."
    done
done
