...
def process_dataset(dataset_name, base_input_dir, output_dir, model_name, model_size):
    ...
    # 2. Data preparation
    datalines = []
    # Use the minimum common number of available samples to avoid IndexError
    num_samples = min(len(d) for d in all_data)
    
    for i in range(num_samples):
        data_index = random.choice(range(len(all_data)))
        
        # Get the associated ratio (if data_index is 0, the ratio is 1.0)
        if data_index == 0:
        ...
        # Output formatting (specific to MATH vs others)
        if config["math_mode"]:
            output_data = f"{cot}\n\nThe final answer is: $\\boxed{{{answer}}}$"
        else:
            output_data = f"{cot}\n\nThe final answer is: \\boxed{{{answer}}}"

        # Build the LLaMA-Factory format
        data = {
...
if __name__ == '__main__':
    ...
    dimensions = ["3b", "7b", "14b"]
    benchmarks = ["boolq", "gsm8k", "math", "piqa"]
    
    for dim in dimensions:
        for bench in benchmarks:
            model_folder_name = f"Qwen2.5-{dim.lower()}-Instruct"
            
            # Build the paths according to the required rule
            # ./outputs/Qwen2.5-DIMENSIONE-Instruct/BENCHMARK/DIMENSIONE
            base_input = f"./outputs/{model_folder_name}/{bench}/{dim.lower()}/"
            
            base_output = f"./outputs/{model_folder_name}/{bench}/"
            
            # Define a clean file name (e.g. "qwen2.5")
            model_base = "qwen2.5"
