...
def extract_piqa_answer(question, model_output, task='cot'):
    # Look inside \boxed{} tolerating extra text (e.g. \boxed{Option 1})
    match = re.search(r'\\boxed\{.*?([12]).*?\}', model_output, re.IGNORECASE)
    if match:
        return match.group(1)
    
    # Look for concluding phrases in the last 150 characters
    end_text = model_output[-150:].lower()
    if "option 1" in end_text or "first option" in end_text: return "1"
    if "option 2" in end_text or "second option" in end_text: return "2"
    
    matches = re.findall(r'\b([12])\b', model_output)
    if matches:
        return matches[-1] # The last occurrence is almost always the final answer
        
    return "" # If there is really no trace of 1 or 2 


def extract_boolq_answer(question, model_output, task='cot'):
    # Work with lower-cased text to simplify matching
    text = model_output.lower()
    
    # 1. Look inside \boxed{} tolerating extra text (e.g. \boxed{True}, \boxed{yes})
    match = re.search(r'\\boxed\{.*?(true|false|yes|no).*?\}', text)
    if match:
        ans = match.group(1)
        return "True" if ans in ['true', 'yes'] else "False"
    
    # 2. Look for concluding phrases in the last 150 characters
    end_text = text[-150:]
    if "the answer is true" in end_text or "the answer is yes" in end_text: 
        return "True"
    if "the answer is false" in end_text or "the answer is no" in end_text: 
        return "False"
    
    # Use \b to ensure whole words (so "notices" is not confused with "no")
    matches = re.findall(r'\b(true|false|yes|no)\b', text)
    if matches:
        last_ans = matches[-1] # The last occurrence is the conclusion of the reasoning
        return "True" if last_ans in ['true', 'yes'] else "False"
        
    return "" # If there is no trace of True/False/Yes/No